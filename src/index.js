...
export default class Autocomplete extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const suggestionsAreEqual = isEqual(
      ...
      <div>
        ...
        >
          <div className={styles.content}>
            {tags.map((tag, idx) => {
              ...
              return (
                <Tag
                  value={valueToRender}
                  key={idx}
                  onDelete={this._onClickDelete.bind(this, idx)}
                />
              );
            })}
            <div
              className={`${
                input ? styles.inputContainer : styles.inputHidden
              }`}
            >
              <input
                type="text"
            ...
            <Suggestions
              ref={node => (this.suggestionsRef = node)}
  ...
  _onChange = ev => {
    ...
    const hasKeys = enterKeys.some(
      enterKey => Key().getCharacter(enterKey) === ev.target.value
    );
    if (hasKeys) return;
  ...
  _onKeyDown = ev => {
    ...
    if (key.is("up")) {
      ...
      this.setState({
        focusedSuggestion:
          focusedSuggestion < 1 ? suggestions.length - 1 : newFocus
      });
    }
  ...
  _handleEnter = (ev, value) => {
    ...
    if (allowCreateTag) {
      let tagToAdd = !hasfocusedsuggestion
        ? value
        : suggestions[focusedSuggestion].label;
      this._addTag(tagToAdd, value);
    } else {
      if (hasfocusedsuggestion) {
        const { label: labelToAdd, value: valueToAdd } = suggestions[
          focusedSuggestion
        ];
  ...
  _addTag = (label, value) => {
    ...
    if (label == "" || (!allowCreateTag && !this._valueMatchSuggestions(label)))
      return;
  ...
  _valueMatchSuggestions = value =>
    this.state.suggestions.some(item => item.label === value);

  _enableCloseOnExit = () => {
    window.document.addEventListener(
      "click",
      this.closeOnClickIfIsDescendant,
      false
    );
  };
  ...
  verifyifshouldShowInput = () => {
    ...
    const newStateInput =
      !props.limitTags || state.tags.length < props.limitTags;
